...
acf(SDL_GDP, main="Seasonal and Non-Seansonal GDP Log Differences")
...
rbind(c("MA(1)","MA(1) & SMA(1)","AR(1)","SAR(1)"),MAE)
...
plot.ts(L_GDP,xlim=c(2010,2021),ylim=c(16,17.5),main="Log GDP predictions (SAR)",ylab="Log GDP")
lines(SARGDP_E,col="red")
lines(SARGDP_L,col="blue")
lines(SARGDP_U,col="blue")
par(mfrow=c(1,1))
...
par(mfrow=c(2,1))
#Non seasonal correlation left in the first quarter and seasonal correlation left in year 1.
acf(SD_UE, main="Seasonal and Non-Seansonal Differences of Unemployment")
#Seasonal correlation left in year 1.
pacf(SD_UE, main="")
par(mfrow=c(1,1))
